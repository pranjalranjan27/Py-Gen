...
def build_charset(use_upper, use_digits, use_symbols, avoid_ambiguous):
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Checkbutton(frame, text="Include UPPERCASE", variable=self.var_upper).grid(row=1, column=0, columnspan=2, sticky="w")
        ttk.Checkbutton(frame, text="Include digits (0-9)", variable=self.var_digits).grid(row=2, column=0, columnspan=2, sticky="w")
        ttk.Checkbutton(frame, text="Include symbols (!@#$...)", variable=self.var_symbols).grid(row=3, column=0, columnspan=2, sticky="w")
        ttk.Checkbutton(frame, text="Avoid ambiguous (l,1,I,0,O)", variable=self.var_avoid_amb).grid(row=4, column=0, columnspan=2, sticky="w")
